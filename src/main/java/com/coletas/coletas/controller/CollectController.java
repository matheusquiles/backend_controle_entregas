...
public class CollectController extends BaseControllerImpl<Collect, Integer> {
	...
	public List<CollectDTO> getDTOByUserAndDate(@RequestBody CollectRequestDTO request){
		try {
			List<CollectDTO> list = service.getDTOByUserAndDate(request.getUserKey(), request.getInitialDate(), request.getFinalDate(), request.getIdSupervisor(), request.getIdEdress());
			return list;
